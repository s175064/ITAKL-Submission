...
% define directory where images will be written
...
for a=1:length(pre_flist)
    ...
    disp((x/91));
        for y=1:109
            for z=1:91
            	% exclude first five volumes when calculating PSD
                psd_img = (pre_img(x, y, z, 5:190));
                % use the pwelch function in matlab to calculate PSD at each voxel
                [pxx, f] = pwelch(psd_img(:), [], [], [], 0.5);
                % get power between 0.01 and 0.1 Hz
                pwr = bandpower(pxx, f, [0.01, 0.1], 'psd');
                % assign pwr value to each voxel
    ...
    % mean normalize all pre PSD values
    ...
    % mean normalize all post PSD values
